...
const ModalFormTarea = () => {
    const {id} = useParams()
    const { alerta, modalFT, tarea, handleModalTarea, mostrarAlerta,submitTarea } = useProyectos()
    const [nombre, setNombre] = useState('')
    const [descripcion, setDescripcion] = useState('')
    const [prioridad, setPrioridad] = useState('')
    const [fechaEntrega, setFechaEntrega] = useState('')
    const [button, setButton] = useState('crear tarea')
    ...
    useEffect(() =>{
        ...
        setPrioridad('')
        setFechaEntrega('')
        setButton('crear tarea')
    },[tarea])

    const handleSubmit = async e => {
        e.preventDefault()
        if([nombre,prioridad,descripcion, fecha].includes('')){
            mostrarAlerta({
        ...
        await submitTarea({id: tareaId, nombre,descripcion,fechaEntrega, prioridad, proyecto: id})
        setTareaId('')
        setNombre('')
        setDescripcion('')
        setPrioridad('')
    ...
    const {msg} = alerta
    return (
        <Transition.Root show={modalFT} as={Fragment}>
